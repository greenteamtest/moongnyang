...
public class media_MyBatisService {
	...
	public List<mediaVO> load_comment(int num) {
		...
		List<mediaVO> list = null;
		try {
			list = dao.load_comment(session, num); // dao에 SqlSession 전송
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
			session.close(); // connection.close()와 비슷, 모든 함수마다 닫기
	...
	public int count_media_comment(String email) {
		SqlSession session = MybatisSessionFactory.getSqlSession(); // 접속 완료
		int rs = 0;
		try {
			rs = dao.count_media_comment(session, email); // dao에 SqlSession 전송
			if (rs > 0) {
				session.commit();
			} else {
				session.rollback();
