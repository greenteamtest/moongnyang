				...
				<nav id="sidebarMenu" class="col-md-3 col-lg-2 d-md-block bg-Secondary sidebar collapse">
					<div class="position-sticky pt-3">
						<ul class="nav flex-column">
							<li class="nav-item">
								<a class="nav-link " aria-current="page" href="mypageServlet?command=managerPage">
									<span data-feather="home"></span>
									Dashboard
								</a>
							</li>
							<li class="nav-item">
								<a class="nav-link" href="mypageServlet?command=managerPageCus">
									<span data-feather="file"></span>
									고객소리함 #소통 #고객
								</a>
							</li>
							<li class="nav-item">
								<a class="nav-link active" href="mypageServlet?command=managerPageBusi ">
									<span data-feather="shopping-cart"></span>
									사업자신청관리
								</a>
							</li>
							<li class="nav-item">
								<a class="nav-link" href="mypageServlet?command=managerPageStaff&email=${loginUser.email}">
					...
					<table class="table table-striped table-sm">
						...
						<tbody>
							<c:forEach var="board" items="${boardList}">
								<tr>
									<td>${board.num}</td>
									<td><a href="mypageServlet?command=businessReadBoard&email=${board.email}">${board.email }</a></td>
									<td>${board.content }</td>
									<td>${board.place_name }</td>
									<td>${board.place_key }</td>
									<td><fmt:formatDate value="${board.writedate}" /></td>
									<td><c:if test="${board.readval==0}">읽음</c:if> <c:if test="${board.readval==1}">읽지않음</c:if></td>
								</tr>
							</c:forEach>
						</tbody>
	...
	<table class="ui compact celled definition table">
		...
		<tfoot class="full-width">
			<tr>
				<th></th>
				<th colspan="4">
					<div class="ui small button">승인</div>
					<div class="ui small button">모두 승인</div>
				</th>
			</tr>
		</tfoot>
	</table>
